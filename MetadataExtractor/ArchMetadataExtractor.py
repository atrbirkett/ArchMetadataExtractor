...
def extract_geotiff_metadata(geotiff_path):
    try:
        with rasterio.open(geotiff_path) as src:
            tags = src.tags()
            # Calculate Ground Sample Distance (GSD) assuming square pixels
            gsd = src.res[0] if src.res[0] == src.res[1] else (src.res[0], src.res[1])
            ...
            return {
                ...
                'FILE_SIZE': str(os.path.getsize(geotiff_path)),
                ...
                'FILE_GSD': f"{gsd} (assuming square pixels)" if isinstance(gsd, tuple) else f"{gsd} per pixel",
            }
